...
Bucket='accesslogsafszbbucket',
)
downloadfile = (response['Contents'][-1]['Key'])
fileiposition = downloadfile.split("/")
filename = fileiposition[8]
print (filename)
#aws s3api get-object --bucket accesslogsafszbbucket --key downloadfile /tmp/filedownload.gzip
client.download_file('accesslogsafszbbucket', downloadfile, '/tmp/'+filename)

def createLogGroupAndStream(loggroup='/aws/logs3tocloudwatch', logstream='streams3tocloudwatch'):
...
def streamevents(events, sequenceToken,loggroup, logstream):
    ...
    print(events, sequenceToken,loggroup, logstream)
